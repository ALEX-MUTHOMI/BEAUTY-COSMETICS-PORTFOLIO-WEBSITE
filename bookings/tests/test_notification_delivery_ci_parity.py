...
@pytest.mark.django_db(transaction=True)
def test_pdf_storage_permission_failure_is_fail_closed_and_redacted(monkeypatch, settings, tmp_path):
    ...
    settings.EMAIL_PROVIDER = "fake"
    settings.RECEIPT_PDF_ARTIFACT_DIR = str(tmp_path)
    booking, _session, _ledger = _confirm_paid_booking("ci-storage-unwritable")

    def fail_pdf(_receipt):
        raise PermissionError("Permission denied: '/app/.local/receipt_artifacts' grace@example.com +254712345678")
    ...
    assert result.sent == 0
    assert result.failed == 1
    assert notification.failure_code == "storage_unwritable"
    assert "redacted-email" in notification.last_error_redacted
    assert "+254" not in notification.last_error_redacted
    assert "/app/.local" not in notification.last_error_redacted
    booking.refresh_from_db()
