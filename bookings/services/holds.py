...
def _selection_fingerprint(*, selection_key, resource_id, starts_at_utc, phone, email):
    material = "|".join(
        [
            str(selection_key),
            str(resource_id),
...
def _safe_response(booking, hold_ttl_minutes):
    ...
    return {
        ...
        "selection": booking.selection_snapshot_json_redacted,
...
class BookingHoldService:
    ...
    def create_bundle_hold(
        ...
        *,
        ...
        request_context = request_context or {}
        redis_client = request_context.get("redis_client")
        _safe_counter(redis_client, "booking:bundle_holds:attempted:10m")
        summary = validate_service_bundle(service_public_ids, client_amount=client_amount)
        resource = cls._get_resource(resource_public_id)
        starts_at_utc = _normalize_start(starts_at)
        ends_at_utc = starts_at_utc + timedelta(minutes=summary.total_duration_minutes)
        phone, email, full_name = cls._normalize_customer_payload(customer_payload)
        request_fingerprint = _selection_fingerprint(
            selection_key=",".join(str(item.service.id) for item in summary.items),
            resource_id=resource.id,
            starts_at_utc=starts_at_utc,
            phone=phone,
            email=email,
        )
        ttl_minutes = _hold_ttl_minutes(_policy(), redis_client)
        ...
        try:
            with transaction.atomic():
                ...
                if not cls._bundle_available_for_start(summary, resource, starts_at_utc):
                    raise ValidationError(GENERIC_UNAVAILABLE)
                booking = cls._create_selection_booking(
                    customer_payload=(full_name, email, phone),
                    resource=resource,
                    starts_at_utc=starts_at_utc,
                ...
                _safe_counter(redis_client, "booking:bundle_holds:created:10m")
        except IntegrityError as exc:
            return cls._recover_integrity_conflict(idempotency_key, request_fingerprint, ttl_minutes, exc)
        return _safe_response(booking, ttl_minutes)

    @classmethod
    def create_full_package_hold(
        cls,
        *,
        ...
        request_context=None,
        client_amount=None,
        client_package_items=None,
    ):
        if client_package_items:
            raise ValidationError(GENERIC_UNAVAILABLE)
        request_context = request_context or {}
        redis_client = request_context.get("redis_client")
        _safe_counter(redis_client, "booking:package_holds:attempted:10m")
        ...
        phone, email, full_name = cls._normalize_customer_payload(customer_payload)
        request_fingerprint = _selection_fingerprint(
            selection_key=summary.full_package.public_id,
            resource_id=resource.id,
            starts_at_utc=starts_at_utc,
            phone=phone,
            email=email,
        ...
        try:
            with transaction.atomic():
                ...
                _safe_counter(redis_client, "booking:package_holds:created:10m")
        except IntegrityError as exc:
            return cls._recover_integrity_conflict(idempotency_key, request_fingerprint, ttl_minutes, exc)
        return _safe_response(booking, ttl_minutes)
    ...
    @staticmethod
    def _get_resource(resource_public_id):
        try:
            resource_query = BookableResource.objects.filter(is_active=True)
            resource = (
                resource_query.filter(id=resource_public_id).first() if resource_public_id else resource_query.first()
    ...
    @staticmethod
    def _bundle_available_for_start(summary, resource, starts_at_utc):
        local_start = starts_at_utc.astimezone(BUSINESS_TZ)
        result = AvailabilityService.get_bundle_available_slots(
            [str(item.service.id) for item in summary.items],
            local_start.date(),
            local_start.date(),
            resource_id=resource.id,
        )
    ...
    @staticmethod
    def _full_package_available_for_start(summary, resource, starts_at_utc):
        local_start = starts_at_utc.astimezone(BUSINESS_TZ)
        result = AvailabilityService.get_full_package_available_slots(
            str(summary.full_package.public_id),
            local_start.date(),
            local_start.date(),
            resource_id=resource.id,
        )
    ...
    def _create_selection_booking(
        *,
        ...
        full_name, email, phone = customer_payload
        customer = CustomerProfile.create_from_plaintext(
            full_name=full_name, email=email, phone=phone, reminder_consent=True
        )
        booking = Booking.objects.create(
            customer_profile=customer,
            service=summary.items[0].service if summary.items else None,
            full_package=summary.full_package,
            resource=resource,
            starts_at=starts_at_utc,
            ends_at=ends_at_utc,
            status=Booking.Status.HELD,
            booking_type=Booking.BookingType.FULL_PACKAGE if summary.full_package else Booking.BookingType.NORMAL,
            hold_expires_at=timezone.now() + timedelta(minutes=ttl_minutes),
            idempotency_key=idempotency_key,
            privacy_policy_accepted_at=timezone.now(),
            no_refund_policy_accepted_at=timezone.now(),
            total_duration_minutes=summary.total_duration_minutes,
            total_price_snapshot=summary.total_price,
            currency_snapshot=summary.currency,
            selection_snapshot_json_redacted=summary.public_snapshot,
        )
        for index, item in enumerate(summary.items):
            BookingServiceItem.objects.create(
                ...
                price_amount_snapshot=item.price,
        ...
        BookingAuditEvent.objects.create(
            ...
            actor_type="customer",
            request_id=request_context.get("request_id"),
            metadata_redacted={"request_fingerprint": request_fingerprint},
        )
        logger.info(
            "booking.selection_hold.created",
            extra={
                "booking_public_id": str(booking.public_id),
