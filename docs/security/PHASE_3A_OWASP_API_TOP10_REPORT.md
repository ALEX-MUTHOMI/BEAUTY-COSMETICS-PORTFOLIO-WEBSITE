...
Status: **Phase 3A blocked - report complete, baseline runtime blockers found.**

This is a report-only security discovery phase. No application code, route behavior,
booking rules, checkout/billing truth rules, gallery security behavior, or staff
auth/session behavior was changed.
...
| A05 Security Misconfiguration | SSL redirect, CORS/CSRF/cookies/admin exposure. | `manage.py check`; Newman CSRF; cookie tests; local route inventory. | ZAP baseline timed out; rerun with stable target and tuned timeout. |
...
| API4 Resource Consumption | Holds, OTP, STK, webhook, gallery uploads, status polling. | Load/latency tests passed; throttles tested. | Add per-endpoint rate-limit inventory doc. |
...
| API8 Misconfiguration | Headers, CORS, CSRF, DEBUG, admin. | Partial tests and settings checks. | ZAP baseline blocked by timeout; rerun in Phase 3A-P. |
...
| ZAP passive headers/config | ZAP attempted | Weak/Blocked | Rerun stable ZAP baseline and triage findings. |
...
Notable operational finding: after long test/ZAP activity, `docker compose ps`
reported the worker as unhealthy and `celery inspect ping --timeout=5` timed out.
This is a Phase 3A blocker and should be treated as an SRE patch/backlog item.
...
1. Stabilize Celery worker health check / responsiveness after long verification runs.
2. Rerun ZAP baseline with stable TLS/target settings and bounded runtime.
3. Add full object-level authorization matrix tests for every object-id route.
4. Add mass-assignment negative tests for ownership/role fields.
5. Add Newman negative cases for staff-only and contact-reveal endpoints.
