...
class MpesaWebhookSerializer(serializers.Serializer):
    ...
    mpesa_receipt_number = serializers.CharField(max_length=64, required=False, allow_blank=True)
    account_reference = serializers.EmailField(required=False)

    def validate(self, attrs):
        if attrs["result_code"] == 0:
            if "amount" not in attrs:
                raise serializers.ValidationError("Successful webhooks require an amount.")
            if attrs["amount"] <= Decimal("0.00"):
                raise serializers.ValidationError("Successful webhook amount must be positive.")
        return attrs
