...
def test_customer_can_only_access_own_checkout():
    owner = User.objects.create_user(
        email="owner@beauty.com", phone_number="+254712200007"
    )
    attacker = User.objects.create_user(
        email="attacker@beauty.com", phone_number="+254712200008"
    )
...
def test_missing_idempotency_key_rejected_for_checkout_creation():
    customer = User.objects.create_user(
        email="missing-idem@beauty.com", phone_number="+254712200009"
    )
    client = secure_client()
